...
def parse():
  ...
  parser.add_argument('dataset', help='pol',type=str)
  ...
  parser.add_argument('--embedding', default=vectors.CCGLOVE,
...
def main():
  ...
  train_seqs, test_seqs, train_labels, test_labels =utils.load_sarc_responses(train_file, test_file, comment_file, lower=args.lower)
  ...
  if args.embed:
    ...
    w2v = vectors.vocab2vecs({word for doc in train_all_docs_tok+test_all_docs_tok for word in doc}, vectorfile=args.embedding)
    train_all_vecs = vectors.docs2vecs(train_all_docs_tok, f2v=w2v, weights=weights)
    test_all_vecs = vectors.docs2vecs(test_all_docs_tok, f2v=w2v, weights=weights)
  else:
    print('Create bongs')
    n = args.n
