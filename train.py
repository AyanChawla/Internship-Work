...
from src.dataset import check_for_required_external_data_files, load_posts, load_sdcq_instances, load_verif_instances
from src.sdqc import Sdqc
from src.util import ScalingMode, arrange_folds_for_k_fold_cross_validation,calculate_post_elmo_embeddings, display_results, filter_instances, generate_folds_for_k_fold_cross_validation, write_answers_json
...
sdqc_train_instances, sdqc_dev_instances, sdqc_test_instances = load_sdcq_instances()
sdqc_all_instances = list(chain(
    sdqc_train_instances, sdqc_dev_instances, sdqc_test_instances))
verif_train_instances, verif_dev_instances, verif_test_instances = load_verif_instances()
verif_all_instances = list(chain(verif_train_instances, verif_dev_instances, verif_test_instances))
...
for repetition_no in range(NUM_CV_REPETITIONS):
    ...
    for i in range(NUM_CV_FOLDS):
        ...
        #sdqc_estimates = sdqc.predict(sdqc_model, posts.keys())
        joblib.dump(sdqc_model,'sqdc_model.pkl')
        ...
        joblib.dump(verif_model,'verif_model.pkl')
        '''verif_estimates = verif.predict(
            verif_model,
            [post.id for post in posts.values() if post.has_source_depth],
            sdqc_estimates)'''
